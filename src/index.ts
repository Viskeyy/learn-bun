...
class WebTransportPolyfill {
    ...
    #connect() {
        ...
        ws.addEventListener('open', (e) => {
            console.debug(`[${this.#sid}] connected`);
        });
        ...
        const reconnectHandler = (closeEvent) => {
            if (closeEvent.code < 2000) {
                ...
                console.debug(
                    `[${this.#sid}]DO NOT reconnect because of error code`
                );
            }
        };
        // reconnect
...
if (typeof globalThis !== 'undefined') {
    if (typeof globalThis.WebTransport === 'undefined') {
        Object.defineProperty(globalThis, 'WebTransport', {
            value: WebTransportPolyfill,
        });
        console.log('[webtransport-polyfill]: WebTransport is polyfilled');
    } else {
        console.debug(
            '[webtransport-polyfill]: WebTransport is native supported'
        );
    }
}
